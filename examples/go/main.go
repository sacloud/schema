package main

import (
	"context"
	"fmt"
	"os"

	"github.com/deepmap/oapi-codegen/pkg/securityprovider"
)

const (
	apiRootURL = "https://secure.sakura.ad.jp/cloud/zone/is1a/api/cloud/1.1"
	iconBody   = ""
)

func main() {
	ctx := context.Background()
	client, err := newClient()
	if err != nil {
		panic(err)
	}

	// createIcon
	created, err := createIcon(ctx, client)
	if err != nil {
		panic(err)
	}
	fmt.Printf("***** Create *****\n%#v\n\n", created)

	// updateIcon
	updated, err := updateIcon(ctx, client, created.ID)
	if err != nil {
		panic(err)
	}
	fmt.Printf("***** Update *****\n%#v\n\n", updated)

	// readIcon
	read, err := readIcon(ctx, client, updated.ID)
	if err != nil {
		panic(err)
	}
	fmt.Printf("***** Read *****\n%#v\n\n", read)

	// List
	listed, err := listIcon(ctx, client)
	if err != nil {
		panic(err)
	}
	fmt.Printf("***** List *****\n%#v\n\n", listed)

	// deleteIcon
	deleted, err := deleteIcon(ctx, client, updated.ID)
	if err != nil {
		panic(err)
	}
	fmt.Printf("***** Delete *****\n%#v\n\n", deleted)
}

func createIcon(ctx context.Context, client *Client) (*Icon, error) {
	res, err := client.CreateIcons(ctx, CreateIconsJSONRequestBody{
		Icon: IconCreateRequestBody{
			Image: iconBody,
			Name:  "from-openapi-generator",
		},
	})
	if err != nil {
		return nil, err
	}

	createResponse, err := ParseCreateIconsResponse(res)
	if err != nil {
		return nil, err
	}
	return &createResponse.JSON201.Icon, nil
}

func updateIcon(ctx context.Context, client *Client, id ID) (*Icon, error) {
	// updateIcon
	name := "from-openapi-generator-upd"
	tags := []string{"tag1", "tag2"}

	res, err := client.UpdateIconById(ctx, id, UpdateIconByIdJSONRequestBody{
		Icon: IconUpdateRequestBody{
			Name: &name,
			Tags: &tags,
		},
	})
	if err != nil {
		return nil, err
	}

	updateResponse, err := ParseUpdateIconByIdResponse(res)
	if err != nil {
		return nil, err
	}
	return &updateResponse.JSON200.Icon, nil
}

func readIcon(ctx context.Context, client *Client, id ID) (*Icon, error) {
	res, err := client.ShowIconById(ctx, id)
	if err != nil {
		return nil, err
	}

	readResponse, err := ParseShowIconByIdResponse(res)
	if err != nil {
		return nil, err
	}
	return &readResponse.JSON200.Icon, err
}

func deleteIcon(ctx context.Context, client *Client, id ID) (*Icon, error) {
	res, err := client.DeleteIconById(ctx, id)
	if err != nil {
		return nil, err
	}

	deleteResponse, err := ParseDeleteIconByIdResponse(res)
	if err != nil {
		return nil, err
	}
	return &deleteResponse.JSON200.Icon, err
}

func listIcon(ctx context.Context, client *Client) (*Icons, error) {
	s := Scope_user
	res, err := client.ListIcons(ctx, IconFindRequest{
		Filter: &IconFindFilter{
			Scope: &s,
			//Name: &[]string {
			//	"CentOS",
			//},
			//TagsName: &[]string {
			//	"category-os",
			//},
		},
		AdditionalProperties: nil,
	})
	if err != nil {
		return nil, err
	}

	listResult, err := ParseListIconsResponse(res)
	if err != nil {
		return nil, err
	}

	return &listResult.JSON200.Icons, nil
}

func newClient() (*Client, error) {
	basicAuthProvider, basicAuthProviderErr := securityprovider.NewSecurityProviderBasicAuth(
		os.Getenv("SAKURACLOUD_ACCESS_TOKEN"),
		os.Getenv("SAKURACLOUD_ACCESS_TOKEN_SECRET"),
	)
	if basicAuthProviderErr != nil {
		panic(basicAuthProviderErr)
	}

	return NewClient(apiRootURL, WithRequestEditorFn(basicAuthProvider.Intercept))
}
